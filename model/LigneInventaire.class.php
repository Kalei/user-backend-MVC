...
/**
 * Description of Ligne Inventaire
 *
...
class LigneInventaire {
    ...
    public function save() {
        ...
        if (!empty($this->id)) {
            $sql = " UPDATE ligneinventaire SET ";
            $set = array();

            foreach ($this->data as $att => $value) {
                $strip_value = $this->stripslashes_array($value);
                if ($att != 'id' && $att != 'id_ligne' && isset($value)) {
                    $set[] = " $att = " . $pdo->quote($strip_value) . " ";
                }
            }

            $sql .= implode(",", $set);
            $sql .= " WHERE id_ligne =" . $this->id;
            ...
            $sql = " INSERT INTO ligneinventaire ";
            ...
            $query = "SELECT id_ligne FROM ligneinventaire ORDER BY id_ligne DESC LIMIT 0 , 1";
            try {
