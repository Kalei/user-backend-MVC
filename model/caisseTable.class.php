...
class caisseTable {
     ...
     public static function getLastCaisse($limit) {
          ...
          $sql = "SELECT * FROM caisse ORDER BY id_caisse DESC LIMIT 0,$limit";
          $stmt = $pdo->query($sql);
     ...
     public static function getCaisseById($id) {
          ...
          $caisse->lignes_caisse = ligneCaisseTable::getLignesByIdCaisse($id);
          return $caisse;
     }
     ...
     public static function getCaissesByDateAchat($date_debut, $date_fin) {
          ...
          $sql = "SELECT * FROM caisse WHERE date_achat BETWEEN '" . $date_debut . " 00:00:00'  AND '" . $date_fin . "  23:59:00' ORDER BY date_achat ASC";
          $stmt = $pdo->query($sql);
          $res = $stmt->fetchAll(PDO::FETCH_ASSOC);
          if (count($res) == 0)
               return false;
     ...
     public static function getCaisseByIdUser($id_user, $limit = 5) {
          ...
          $sql = "SELECT * FROM caisse WHERE id_user=" . $id_user." LIMIT 0,$limit";
          $stmt = $pdo->query($sql);
          $res = $stmt->fetchAll(PDO::FETCH_ASSOC);
          if (count($res) == 0)
               return false;
