...
// private function => fonction réservée à l'objet
     ...
     public function setTotaux() {
		...
		if(count($this->lignes_caisse)>0) {
			...
			$this->total_prix_public_ttc = round($this->total_prix_public_ttc,2);
			$this->total_prix_public_ht = round($this->total_prix_public_ht,2);
			$this->total_caisse_ttc = round($this->total_caisse_ttc,2);
...
//••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••
          ...
          if (key_exists($key_ligne, $this->lignes_caisse)) {
               ...
               if ($ligne_actuelle->id_etat_ligne_commande == 52) {
                    ...
                    if ($addedArticle->stock >= $nb) // le stock absorbe nouvelle quantité totale
                         ...
                         $nouvelle_ligne = array("id_caisse" => $this->getCaisseId(), "reference" => $addedArticle->reference, "quantite" => $nb_a_commander, "id_etat_ligne_commande" => 21,
						  "montant_remise_unitaire_ht" => 0, "montant_unitaire_ht" => $prix_public_ht, "prix_public_ht" =>$prix_public_ht);
                         $this->lignes_caisse[] = new LigneCaisse_2($nouvelle_ligne);
                    }
               }
               $this->setTotaux();
               return true;
          ...
          if (!empty($refAdd)) {
               ...
               if ($addedArticle != FALSE) {
                    ...
                    if (count($key_lignes_exists) == 0) {
						 ...
						 $prix_public_ht = round($addedArticle->prix_public/((100+$addedArticle->tva)/100),2);
                         $nouvelle_ligne = array("id_caisse" => $this->getCaisseId(), "reference" => $refAdd, "quantite" => 1, "id_etat_ligne_commande" => $etat_nouvelle_ligne,
						 	 "montant_remise_unitaire_ht" => 0, "montant_unitaire_ht" => $prix_public_ht, "prix_public_ht" =>$prix_public_ht);
                         $this->lignes_caisse[] = new LigneCaisse_2($nouvelle_ligne);
                         $this->setTotaux();
                    } else {
	...
	// 2. Déduit setTotalRemiseHT()
			...
			/*$this->data = array(
               ...
               foreach ($this->lignes_caisse as $ligne) {
                    $values = array(
						...
						"prix_public_ht" => $ligne->getPrixPublicHT() ,
     ...
     public function isPaiementAchived($list_paiement) {
          ...
          if (number_format(floatval($total_paiement), 2) == number_format(floatval($this->total_caisse_ttc), 2)) {
               return 0;
          }

          $diff = number_format(floatval($this->total_caisse_ttc), 2) - number_format(floatval($total_paiement), 2);
          return $diff;
     }
     ...
     public function getTotalCaisseHT() {
          return $this->total_caisse_ht;
     }

     public function getTotalCaisseTTC() {
