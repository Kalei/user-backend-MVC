    ...
    public function save() {
        ...
        if (!empty($this->id)) {
            ...
            foreach ($this->data as $att => $value) {
                ...
                if ($att != 'id' && $att != 'id_role_user' && $value) {
                    $set[] = " $att = " . $pdo->quote($strip_value) . " ";
                }
            }

            $sql .= implode(",", $set);
            $sql .= " WHERE id_role_user =" . $this->id;
            ...
            $query = "SELECT id_role_user FROM role_user ORDER BY id_role_user DESC LIMIT 0 , 1";
            try {
