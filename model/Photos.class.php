    ...
    public function save() {
        ...
        if (!empty($this->id)) {
            ...
            foreach ($this->data as $att => $value)
                if ($att != 'id' && $att != 'id_photo' && $value)
                    $set[] = " $att = " . $pdo->quote($value) . " ";

            $sql .= implode(",", $set);
            $sql .= " WHERE id_photo =" . $this->id;
            $last_id = $this->id;
            try {
                $pdo->exec($sql);
            ...
            $query = "SELECT id_photo FROM photos ORDER BY id_photo DESC LIMIT 0 , 1";
            try {
